...
public class YieldlabBidder implements Bidder<Void> {
    ...
    private String makeUrl(ExtImpYieldlab extImpYieldlab, BidRequest request) {
        ...
        getDsaRequestParams(request).forEach(uriBuilder::addParameter);
    ...
    private static Map<String, String> getDsaRequestParams(BidRequest request) {
        if (!hasDsa(request)) {
            return Map.of();
        }
        ...
        if (dsa.getTransparency() != null && !dsa.getTransparency().isEmpty()) {
            dsaRequestParams.put("dsatransparency", getTransparenciesString(dsa.getTransparency()));
        }
    ...
    private static String getTransparenciesString(List<ExtRegsDsaTransparency> transparency) {
